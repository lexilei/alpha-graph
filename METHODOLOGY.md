...
The "Sharpe 1.91" number is not retracted in the codebase yet (the extension still runs and still produces it on the 23-month subset), but it is no longer claimed as a real result. The production trading wiring that called it "Anti-Momentum" was removed in the 2026-04-15 cleanup.
...
4. **There is no demonstration of LLM-derived alpha.** The multi-agent pipeline was run as a one-period snapshot, never accumulated enough monthly snapshots for longitudinal evaluation, and was removed in the 2026-04-15 cleanup.
5. **There is no demonstrated edge from the knowledge graph spillover signal.** It hurt OOS returns when enabled and was removed in the 2026-04-15 cleanup.
