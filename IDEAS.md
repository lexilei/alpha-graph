...
| ID | name | status | 内容 |
|----|------|--------|------|
| U3 | `research_automation` | ongoing(碎片已在) | **研究自动化层(用户登记 2026-07-22;行业语境 = trading firm 的 "Research Automation" 岗:自动信号搜索 / 迭代周期加速 / LLM-agent 做研究 / HPC 管线)**。本账本的版本必须先解决一个结构性冲突:**"自动生成几千个候选 + 大规模自动回测"正是账本 N 税要杀的东西** — E[max]-null 随 N 涨,面板功效不涨,盲目自动化只是更快地烧掉面板。所以自动化的正确落点是 **look 的单价和质量,不是 look 的数量**:(a) idea 采集/排序自动化 — `ideation_agents_2026-07-17`(57→12 入 Inbox,去重逮住 2 具尸体)是既有实例;(b) look 之外的一切:数据 QA gate、PIT 对齐、prereg 草稿生成、pinned diagnostics、报告落笔 — 已有大半(qa_singles、judge 管道、commit-then-run);(c) 大规模自动搜索仅在两个条件同时满足时合法:选择修正诚实入账(sel-null 标签机器已有)且功效在(新数据/新 universe,即 U1 之后);(d) agent 轨迹 → 可复用工具的沉淀(用户 Reasoning Primitive Induction 研究线的工程版)。**镜像价值**:这个账本本身就是"带纪律的研究自动化"的活样本 — 判决机制 + agent 执行 + 全留痕,恰是该岗位 JD 描述的系统;个人研究(Learning to Construct / RPI)与账本工程互为证据。不占 look,持续演进,无预算无 deadline。 |
...
| U1 | `universe_sharadar` | **in-progress(2026-07-25 订阅已购)** | **进展 2026-07-25**:SFA Personal 月付已买(7/25–8/25,发票在案),API key 验通;实测截面 **5,785 名/日**(现面板 857,免费层只有 30 名 × 2018Q4)。**踩到的坑(记录以免重犯)**:`alpha_graph.data.sharadar` 全部 1583 行测试都是 mock,从未打过真实 API —— 端点(v1 bulkdownloads → 401)、等值/多值筛选算子(`.eq`/`.in[]` 均被拒)、文件格式(假设 Parquet,实为 ZIP+CSV)、响应结构四样全错。真实流程已实证:`GET /api/v3/datatables/SHARADAR/{T}.json?qopts.export=true` 带 `X-Api-Token` → `datatable_bulk_download.file{link,status}`,status `fresh` 后取预签名 S3 的 ZIP。**处置**:先用 `scripts/fetch_sharadar_snapshot.py` 抢救原始数据(整表、带 sha256 与 provenance、可续传),审计版下载器与 QA 的 membership gate 另行修复。**首次盲 QA 判决(2026-07-25,快照 `2026-07-25-v2`,67 分钟):`gate_status=FAIL`,14 条 issue** —— 这是 gate 在按设计工作,不是数据坏了。分两类:(a) **裁定积压**(尚无任何 approvals 文件):known_identity_cases 0/14、listing_issuer_registry 0/1181、membership_difference 0/135、price_defect 255(=2 非正价 + 253 空成交量,与入库记录逐一吻合)、price_jump 64,177、identity_quarantine 101。(b) **阈值是按 857 名标普面板标定的,全市场跑出来必然不合**:member_day_price_coverage 92.0%/84.7%(bar 99%/98%)、departed_identity_coverage 68.3%(bar 98%)、sep_row_quarantine 6,091 行零匹配、membership_jaccard 71.6%(bar 99%)。**membership_jaccard 已查证为机制性假警报,勿据此怀疑成员名单**:vendor_only 是 `APC1`/`AGN1`/`ALTR1` 这类 Sharadar 的 **ticker 复用消歧后缀**(与本地 `APC`/`AGN` 同一证券),而身份映射恰恰是那条 0/1181 未提供的 gate;叠加 vendor 的 historical 快照是**季度频**(28 年仅 114 个日期,对比时最多 73 天陈旧)。jaccard 随陈旧度下降单调上升(2007 年 0.673 / 73d → 2014 年 0.758 / 69d → 2018 年 0.843 / 24d),与机制解释一致。**下一步**:先补身份映射与 approvals,再重判;price_jump 的 `verify_price_jumps` 是逐条 Python 循环 × 67 万行 ACTIONS 扫描(O(jumps×actions)),全市场 6.4 万条 jump 使其成为 QA 主要耗时,须向量化。**教训**:mock-only 的测试套件对"能不能真的下到数据"零证据,付费窗口开始前应先用免费层打通传输。原始计划如下 —— 扩到 Sharadar 全市场含退市 PIT universe(2000+ 名)。修掉每行 C 因子的 survivorship caveat;把栖息地搬到异象实际存活的小盘/难套利段(McLean-Pontiff 残余所在)。第一个标定测试:C17 SUE/PEAD 全市场重跑(PEAD 小盘集中,兼作 C17 的 out-of-design 确认)。成本:数据订阅 + `sharadar.py`/`sharadar_qa.py` 扩展 + 面板重建,约数天到一周。**这是当前最高杠杆的一项投资,优先级高于队列中任何单因子。** **衰减文献定论(2026-07-15,`data/reference/decay_literature/`)**:U1 把测试从"发表异象拟合残留 ≈ 0 的角落"(最大市值/最高流动/低特质波动,McLean-Pontiff Table 8:idio +4.05 p<.001、size −1.49 p=.013、$vol −1.67 p<.01)搬进残余 alpha 实际存活的栖息地,等价于给队列每个难套利异象恢复 +1 P;且 US 是唯一有可靠发表后衰减的市场(Jacobs-Müller 2020)— 本面板是全球最坏栖息地,U1 是对它唯一的结构性修复。 |
...
| dividend-seasonality | **C36 rejected(反号)** | 前置成本检查通过后开族(精度 79.1%/召回 97.7%,17.1bp);**C36 DivYieldST:sn incr t=−1.72(HAC −1.57)与钉死的正号相反 → rejected**——现代大盘无 H-S 分红月溢价支持,反号不构成发现(sign-mining 禁区,~8.5% 单 look 零概率);DivSeason member 2 继续冻结(6.8bp 边际成本 + 姊妹假设已败,花它的理由实质塌了);\|t\|≥1.5 使家族到期不自动关,但无修正案不许再看 |
